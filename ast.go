...
func (s *DropFieldStatement) String() string {
	var buf bytes.Buffer
	buf.WriteString("DROP FIELD ")
	buf.WriteString(QuoteIdent(s.Name))
	buf.WriteString(" FROM ")
	buf.WriteString(QuoteIdent(s.Measurement))
	if s.Database != "" {
		buf.WriteString(" ON ")
...
// RequiredPrivileges returns the privilege required to execute a DropFieldStatement.
func (s *DropFieldStatement) RequiredPrivileges() (ExecutionPrivileges, error) {
	return ExecutionPrivileges{{Admin: false, Name: s.Database, Privilege: WritePrivilege}}, nil
}
...
func (s *RenameFieldStatement) String() string {
	var buf bytes.Buffer
	buf.WriteString("ALTER MEASUREMENT ")
	buf.WriteString(QuoteIdent(s.Measurement))
	if s.Database != "" {
		buf.WriteString(" ON ")
		buf.WriteString(QuoteIdent(s.Database))
	}
	buf.WriteString(" RENAME FIELD ")
	buf.WriteString(QuoteIdent(s.OldName))
	buf.WriteString(" TO ")
	buf.WriteString(QuoteIdent(s.NewName))
	return buf.String()
}

// RequiredPrivileges returns the privilege required to execute a RenameFieldStatement.
func (s *RenameFieldStatement) RequiredPrivileges() (ExecutionPrivileges, error) {
	return ExecutionPrivileges{{Admin: false, Name: s.Database, Privilege: WritePrivilege}}, nil
}
